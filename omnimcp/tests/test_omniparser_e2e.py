...
def test_environment():
    ...
    try:
        if not provider.is_available() and not os.environ.get("AWS_ACCESS_KEY_ID"):
            ...
            logger.warning("Either start a local server, set OMNIPARSER_URL, or add AWS credentials")
        ...
        if not os.environ.get("AWS_ACCESS_KEY_ID"):
            ...
            pytest.skip("No OmniParser server available and no credentials to deploy one")
...
def test_server_availability(test_environment):
    ...
    client = OmniParserClient(provider.server_url)
    assert client.check_server_available(), "OmniParser server not responsive after deployment"
...
def test_image_parsing(test_environment):
    ...
    assert client.check_server_available(), "OmniParser server not available for parsing test"
