...
def sample_elements() -> list[UIElement]:
    ...
    return [
        UIElement(id=0, type="text_field", content="", bounds=bounds, attributes={"label": "Username:"}),
        UIElement(id=1, type="text_field", content="", bounds=bounds, attributes={"is_password": True, "label": "Password:"}),
        UIElement(id=2, type="checkbox", content="Remember Me", bounds=bounds, attributes={"checked": False}),
...
def test_plan_action_step1_type_user(mocker, sample_elements):
    ...
    assert sample_elements[0].to_prompt_repr() in messages[0]['content'] # Check element rendering
